def clean_dataset(df):
    ...
    # Category mapping
    category_map = {
        'food & dining': 'Food & Dining', 'shopping': 'Shopping', 'entertainment': 'Entertainment',
        'utilities': 'Utilities', 'transportation': 'Transportation', 'transport': 'Transportation',
        'healthcare': 'Healthcare', 'education': 'Education', 'savings': 'Savings', 'rent': 'Rent',
        'others': 'Others'
    }
    # Using .fillna('Others') handles anything that didn't match our map
    cleaned_df['category'] = cleaned_df['category'].str.lower().map(category_map).fillna('Others')

    # Payment method mapping
    payment_map = {
        'gcash': 'GCash', 'g-cash': 'GCash', 'maya': 'Maya', 'credit card': 'Credit Card',
        'debit card': 'Debit Card', 'bank transfer': 'Bank Transfer', 'bdo online': 'BDO Online',
        'cash': 'Cash'
    }
    cleaned_df['payment_method'] = cleaned_df['payment_method'].str.lower().map(payment_map).fillna('Cash')

    # Status mapping
    status_map = {
        'paid': 'Paid', 'pending': 'Pending', 'cancelled': 'Cancelled', 'refunded': 'Refunded'
    }
    cleaned_df['status'] = cleaned_df['status'].str.lower().map(status_map)

    # --- Numeric Handling & Filling ---
    cleaned_df['budget_limit_php'] = pd.to_numeric(cleaned_df['budget_limit_php'], errors='coerce').fillna(0.0)
    ...
    cleaned_df.loc[(cleaned_df['amount_php'] < 0) & (cleaned_df['status'].isna()), 'status'] = 'Refunded'
    ...
    cleaned_df = cleaned_df[~((cleaned_df['status'].astype(str).str.upper() == 'PAID') & (cleaned_df['amount_php'].isna()))]
    ...
    # --- Outlier Handling ---
    cleaned_df.loc[(cleaned_df['status'] == 'Refunded') & (cleaned_df['amount_php'] > 0), 'amount_php'] = -cleaned_df['amount_php']

    # Cast categorized items back into strict Pandas categories
    strict_categories = ['account_type', 'category', 'subcategory', 'payment_method', 'necessity_type', 'status']
    for cat_col in strict_categories:
        cleaned_df[cat_col] = cleaned_df[cat_col].astype('category')

    # --- Export cleaned dataset ---
    cleaned_df.to_csv('src/data/financial_expenses_dataset_cleaned.csv', index=False)
